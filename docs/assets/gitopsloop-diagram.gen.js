// Generator for the gitops-repo loop schematic (light + dark SVGs) — the
// companion to devloop-diagram.gen.js (the app loop), sharing its tokens per
// ./STYLEGUIDE.md. Regenerate after editing card text / palette:
//   node docs/assets/gitopsloop-diagram.gen.js docs/assets
// Emits gitopsloop-light.svg + gitopsloop-dark.svg (embedded in README via
// <picture>).
//
// Same loop, minus the build: a commit to the gitops repo converges the live
// cluster without any image in the path. The app loop's image bumps land on
// the deploy branch this loop maintains, so the two diagrams meet at card 02.
const fs = require('fs');
...
// (git-deploy-controller in flywheel-system, kustomize-controller in
// flux-system) — mono promises literal.
const cards = [
  { n: '01', role: 'you',  font: 'sans', title: 'you edit manifests + git commit',  sub: ['in your gitops repo — apps/, infra/, builders/'] },
  { n: '02', role: 'comp', font: 'mono', title: 'git-deploy-controller',            sub: ['pushes your branch to the in-cluster git-server;', 'maintains flywheel/local-deploy (+ image bumps)'] },
  { n: '03', role: 'flux', font: 'mono', title: 'kustomize-controller',             sub: ['applies your infra/ builders/ apps/ overlays,', 'pruning whatever the commit removed'] },
  { n: '04', role: 'you',  font: 'sans', title: 'the cluster matches your repo',    sub: ['a few seconds after the commit — no push, no CI'] },
];
// transition pill keyed by the index of the card it leads INTO
const pills = { 1: 'via the /workspaces bind-mount' };
...
// The edit/commit (stage 01) happens on the host; git-deploy-controller is an
// in-cluster pod that reads the host worktree over the /workspaces bind-mount
// — that read is the boundary crossing the pill labels. Drawn behind the
// cards as a recessed, neutrally-tinted zone with a corner label.
...
function header(p) {
  ...
  const text = 'THE SAME LOOP, MINUS THE BUILD';
  const tx = CX;
  return `<g>
...
fs.writeFileSync(`${dir}/gitopsloop-light.svg`, build('light'));
fs.writeFileSync(`${dir}/gitopsloop-dark.svg`, build('dark'));
console.log(`W=${W} H=${TOTAL_H} panelH=${PANEL_H} cards=${layout.length}`);
console.log('wrote gitopsloop-light.svg, gitopsloop-dark.svg');
