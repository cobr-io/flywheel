...
> decrypts and comes up with no key handoff. See the
> [onboarding guide](https://example.invalid/flywheel/blob/main/docs/guides/onboarding.md)
> for the full flow.
...
scanner, see the [onboarding guide](https://example.invalid/flywheel/blob/main/docs/guides/onboarding.md):
`clusters/local/age.key` is committed on purpose and must be allowlisted or it
reads as a leaked key.
...
You're not locked in: `apps/` and `infra/` are plain Kustomize, so you can run
this cluster with stock Flux and **no `flywheel` binary** by hand-authoring a
vanilla entrypoint yourself. The
[flywheel-free bring-up guide](https://example.invalid/flywheel/blob/main/docs/guides/flywheel-free-bringup.md)
walks through it.
